window.onload = function () {
  ...
  function collide(elem_a, elem_b) {
    ...
    if (dist <= elem_a.rad + elem_b.rad) {
      var vel_ax = (elem_a.vel_x * (elem_a.radius – elem_b.radius) + (2 * elem_b.radius * elem_b.vel_x)) / (elem_a.radius + elem_b.radius);
      var vel_ay = (elem_a.vel_y * (elem_a.radius – elem_b.radius) + (2 * elem_b.radius * elem_b.vel_y)) / (elem_a.radius + elem_b.radius);
      var vel_bx = (elem_b.vel_x * (elem_b.radius – elem_a.radius) + (2 * elem_a.radius * elem_a.vel_x)) / (elem_a.radius + elem_b.radius);
      var vel_by = (elem_b.vel_y * (elem_b.radius – elem_a.radius) + (2 * elem_a.radius * elem_a.vel_y)) / (elem_a.radius + elem_b.radius);
      elem_a.vel_x = vel_ax;
      elem_a.vel_y = vel_ay;
      elem_b.vel_x = vel_bx;
      elem_b.vel_y = vel_by;
